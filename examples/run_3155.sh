...
"$HERE/run_reconplot.R" --source savana --input "$IN" --outdir "$OUT/per_chromosome" \
  --regions all --layout separate --format pdf,png --write-tables

"$HERE/run_reconplot.R" --source savana --input "$IN" --outdir "$OUT/genome_wide" \
  --regions all --layout together --width 34 --height 4 --format pdf,png \
  --extra "size_text=4,size_chr_labels=5,scale_ticks=50000000"

"$HERE/run_reconplot.R" --source savana --input "$IN" --outdir "$OUT/focus" \
